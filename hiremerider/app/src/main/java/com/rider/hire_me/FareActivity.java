...
//        controller
        ...
        if (currentTrip != null) {
            ...
            try
            {
                ...
                if(promo>0){
                    ...
                    actualAmount.setText(String.format(Locale.ENGLISH,controller.currencyUnit()+"%.02f", Float.parseFloat(currentTrip.getTripPayAmount())));
                    bigAmount.setText(String.format(Locale.ENGLISH,controller.currencyUnit()+"%.02f", tripCalculated));
                }else{
                    actualAmount.setText(String.format(Locale.ENGLISH,controller.currencyUnit()+"%.02f", Float.parseFloat(currentTrip.getTripPayAmount())));
                    bigAmount.setText(String.format(Locale.ENGLISH,controller.currencyUnit()+"%.02f", Float.parseFloat(currentTrip.getTripPayAmount())));
                }
    ...
    public void fareApplyPromoCode(View view) {
        ...
        try{
            ...
            imm.hideSoftInputFromWindow(tvPromocode.getWindowToken(), 0);
        }catch (Exception e){
            e.printStackTrace();
        }
    ...
    public void farePromoCode(View view) {
        if (promoLayout.getVisibility() == View.INVISIBLE) {
            ...
            tvPromocode.setFocusable(true);
            try {
                InputMethodManager imm = (InputMethodManager) getSystemService(Context.INPUT_METHOD_SERVICE);
                imm.showSoftInput(tvPromocode, InputMethodManager.SHOW_FORCED);
            }catch (Exception e){
                e.printStackTrace();
            }
        } else {
...
//        curURL = NSURL(string: "\(trip_url)\(UPDATE_TRIP)?api_key=\(self.appDelegate.apikey)&trip_id=\(self.appDelegate.tripid)&trip_pay_mode=\(pay_type)&trip_pay_status=Paid")!
      ...
      /*  } else {
        ...
        if (isPromocodeUsed) {
            ...
            params.put("trip_promo_amt", String.format(Locale.ENGLISH,"%.02f", amtPromo));
        }
        else{
            rlPromoAmount.setVisibility(View.GONE);
        }
...
//        2017-01-25 07:02:31
        ...
        if (isPromocodeUsed) {
            ...
            params.put("pay_promo_amt", String.format(Locale.ENGLISH,"%.02f", amtPromo));
            fare=fare-amtPromo;
    ...
    public void handlePromoCode(String promoResponse) {
        ...
        try {
            ...
            if(isPromocodeUsed) {
                promoCodePrice.setVisibility(View.VISIBLE);
                promoCodeText.setVisibility(View.VISIBLE);
                bigAmount.setText(controller.currencyUnit() +String.format(Locale.ENGLISH,"%.02f ", amtAfterApplyPromo));
                rlPromoAmount.setVisibility(View.VISIBLE);
                promoCodePrice.setText(controller.currencyUnit()+String.format(Locale.ENGLISH,"%.02f ", amtPromo));
